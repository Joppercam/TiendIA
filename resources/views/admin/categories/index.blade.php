    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('admin.categories.create') }}" class="bg-indigo-600 text-white py-2 px-4 rounded hover:bg-indigo-700">
                    Nueva Categoría
                </a>
            </div>
            ...
            <table class="min-w-full bg-white">
                <thead>
                    <tr>
                        ...
                        <th class="py-3 px-6 text-left bg-gray-100">Categoría Padre</th>
                    ...
                    @foreach($categories as $category)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-4 px-6 border-b border-gray-200">
                                @if($category->image)
                                    <img src="{{ asset('storage/categories/thumbnails/' . $category->image) }}" alt="{{ $category->name }}" class="h-10 w-10 object-cover rounded">
                                @else
                                    <div class="h-10 w-10 bg-gray-200 rounded flex items-center justify-center text-gray-500">N/A</div>
                                @endif
                            </td>
                            <td class="py-4 px-6 border-b border-gray-200">{{ $category->name }}</td>
                            <td class="py-4 px-6 border-b border-gray-200">{{ $category->parent ? $category->parent->name : 'N/A' }}</td>
                            <td class="py-4 px-6 border-b border-gray-200">
